...
export class CreateCustodyTables1700000000001 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.createIndex(
      'custody_records',
      new Index('IDX_custody_records_parcelId', ['parcelId']),
    );

    await queryRunner.createIndex(
      'custody_records',
      new Index('IDX_custody_records_fromParty', ['fromParty']),
    );

    await queryRunner.createIndex(
      'custody_records',
      new Index('IDX_custody_records_toParty', ['toParty']),
    );

    await queryRunner.createIndex(
      'custody_records',
      new Index('IDX_custody_records_timestamp', ['timestamp']),
    );

    await queryRunner.createIndex(
      'custody_records',
      new Index('IDX_custody_records_blockchainTxHash', ['blockchainTxHash']),
    );

    // Create indexes for custody_queue
    await queryRunner.createIndex(
      'custody_queue',
      new Index('IDX_custody_queue_status', ['status']),
    );

    await queryRunner.createIndex(
      'custody_queue',
      new Index('IDX_custody_queue_createdAt', ['createdAt']),
    );

    await queryRunner.createIndex(
      'custody_queue',
      new Index('IDX_custody_queue_retryCount', ['retryCount']),
    );

    await queryRunner.createIndex(
      'custody_queue',
      new Index('IDX_custody_queue_processedAt', ['processedAt']),
    );
  }
